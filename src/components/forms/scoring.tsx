...
import { updateBatsmanRun } from "@/lib/actions/batting.action";
import { updateBowlerRun } from "@/lib/actions/bowling.action";
import { updateInningsRuns } from "@/lib/actions/innings.action";
...
export default function Scoring({
  ...
  inningsDetails: inningsWithMatch;
  battingTeamPlayers: player[];
  bowlingTeamPlayers: player[];
  strikers: battingScorecard[];
  bowler: bowlingScorecard | null;
}) {
  let ballInOver;
  let totalOvers;
  let totalBalls;
  ...
  if (bowler && inningsDetails) {
    ballInOver = inningsDetails?.matchId.ballInOver;
  ...
  const [selectedStriker, setSelectedStriker] =
    useState<battingScorecard | null>(null);
  ...
  const handleScoreUpdate = async (score: number) => {
    ...
    if (selectedStriker && bowler && inningsDetails) {
      ...
      const isSix = score === 6;
      const updateBatsmanScore = await updateBatsmanRun(
        inningsDetails._id,
        selectedStriker?.playerId._id,
        isFour,
        isSix,
        score
      );
      if (!updateBatsmanScore.success) {
        alert("Error updating batsman score");
      }
              ...
              {strikers.map((striker, id) => (
                <TableRow
                  ...
                  className={`cursor-pointer ${
                    selectedStriker?.playerId._id === striker.playerId._id
                      ? "bg-orange-400 hover:bg-orange-400"
                      : ""
                  }`}
                >
                  <TableCell className="font-medium">
                    {striker.playerId.playerName}
                  </TableCell>
